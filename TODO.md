...
- [x] **PolyHive pipeline watchdog + Phase 2 framework** — Root cause: pipeline stalled at Stage 2 for 8 days (03-30 through 04-05) because LLM-to-LLM exec handoffs were blocked by OpenClaw exec security policy (`askFallback=deny` in cron sessions). Solution: deployed `pipeline-runner.py` watchdog running via cron at 14:15, 15:00, 15:30, 16:00 UTC — advances pipeline deterministically without relying on LLM exec calls. Also: (1) added data enrichment fields to paper-trade.py for Phase 2 validation; (2) fixed graduation criteria from 15 to 30/90 trades (statistically sound thresholds); (3) deployed `validate-thesis.py` with Sunday 10:00 UTC cron; (4) updated all relevant RUNBOOKs (analyst, contrarian, quant, risk). Verified: stalled 04-05 pipeline completed in <60s, 5 new trades added. Ledger: 10 total. mhive briefed. _(2026-04-05)_
- [x] **Merge upstream v2026.4.5 + deploy** — Merged 2,854 upstream commits (v2026.4.1 → v2026.4.5). Security audit: all SAFE. One merge conflict in `src/agents/pi-embedded-runner/run/attempt.ts` (resolved: accepted upstream's `buildEmbeddedAttemptToolRunContext` spread helper). Key changes: video generation (Runway, xAI, Alibaba, fal providers), music generation (ComfyUI), memory wiki plugin (Obsidian import, search bridge, dashboard reports), dreaming/REM sleep phases, Bedrock Mantle provider (OpenAI-compat), Fireworks + StepFun + MiniMax TTS providers, QA Lab extension, iOS exec approval, Lobster TaskFlow mode, TinyFish browser automation, Ukrainian/Turkish/Polish i18n, GPT personality overlay. Pre-existing upstream TS error in `update-command.ts` (optional `root` passed as `string`). Migrated legacy `talk.apiKey` → `talk.providers.elevenlabs.apiKey` on both staging and VPS. `OPENCLAW_BUNDLED_PLUGINS_DIR=/app/dist/extensions` workaround still required. _(2026-04-06)_
- [x] **PolyHive pipeline fixes + Thaleia allowlist (2026-04-11)** — Three fixes: (1) `verify-outcomes.py` — root cause: script queried `?clob_token_ids=` which returns empty for resolved/archived Polymarket markets. Fixed to query `/events?slug=<event-slug>` and use `outcomePrices: ["1","0"]` as price-settlement signal (works even before official `resolved=true`). Resolved 14 backlogged trades in one run. (2) `paper-trade.py` — added `PAPER_EXPOSURE_CAP = 500.0` hard stop: if total open exposure ≥ $500, no new trades entered that day. Prevents runaway position stacking on active match days. (3) Telegram allowlist — Thaleia (ID `6935745875`) added to mhivebook `groupAllowFrom` by mhive; documented in `workspace/USER.md` on VPS and local MEMORY.md. Ledger state: 18 closed (8W/6L recent-phase), 0 open, +$130.15 total PnL. TH-001: 18/30 trades.
- [x] **Merge upstream v2026.4.9 + deploy** — Merged 1,988 upstream commits (v2026.4.5 → v2026.4.9). Security audit: all SAFE. One merge conflict in `docs/tools/video-generation.md` (resolved: accepted upstream's new provider capability modes + live tests sections). Key changes: 4 new extension packages (arcee, memory-wiki, webhooks, plugin-sdk), new deps (acpx, @aws-sdk/client-bedrock, undici), browser control hardening, workspace .env injection blocked, untrusted plugin auth hijacking blocked, Matrix sync outage containment, Nostr runtime deps, parallel character eval runs, QA live provider overrides, control UI locale refreshes (fr/ja/tr/uk/pl/id). Dockerfile removed `OPENCLAW_BUNDLED_PLUGINS_DIR` env var (override in docker-compose still required). `docker-compose.staging.yml` deleted by upstream again (restored locally). Pre-existing upstream lint errors in discord/feishu extensions (not ours). _(2026-04-08)_
- [x] **Merge upstream v2026.4.11-beta.1 + deploy** — Merged 1,244 upstream commits (v2026.4.9 → v2026.4.11-beta.1). Security audit: all SAFE. Clean merge, no conflicts. Key changes: massive import cycle cleanup (dozens of `fix(cycles):` commits splitting heavy barrels into narrow runtime seams for faster startup), MS Teams federated credential support (certificate + managed identity auth, SSO, reactions, message actions), new Codex extension (pluggable agent harness, app-server protocol, OAuth), axios pinned to 1.15.0 + plugin install dependency denylist, SSRF hostname validation hardening, browser sandbox noVNC auth gating, expanded host env security denylist, basic-ftp advisory patch, MiniMax OAuth fix, Ollama thinking support, Slack ACP block text, plugin manifest activation/setup descriptors, Matrix MSC4357 live streaming, plugin text transforms, QA Lab proxy capture stack. **Breaking:** sandbox mode now defaults to requiring Docker — set `agents.defaults.sandbox.mode=off` on both staging and VPS (no Docker-in-Docker). `OPENCLAW_BUNDLED_PLUGINS_DIR=/app/dist/extensions` override still required. _(2026-04-11)_
...
- [ ] **PolyHive 30-trade interim checkpoint** — `[Layer 2 — Agent]`
  - At **18 closed trades** as of 2026-04-11 (14 resolved today by verify-outcomes.py fix). Need 30 closed for first statistical checkpoint.
  - Current win rate: 8W / 6L (from recent-phase trades) — below 55% thesis threshold. Watching for trend.
...
- [ ] **Percy daily memory notes not being written** — `[Layer 2 — Agent]`
  - RUNBOOK Afternoon Protocol step 5 added 2026-04-08, but Percy has not written memory/YYYY-MM-DD.md for Apr 9, 10, or 11
  - mhive's stall-alert check depends on this file — false alerts will continue until Percy complies
  - Notify Percy via mhive. If not resolved by next session, inspect Percy's actual session transcript to diagnose why step 5 is not executing
...
- **PolyHive verify-outcomes API pattern (2026-04-11):** Polymarket's `gamma-api.polymarket.com/markets?clob_token_ids=...` endpoint returns empty for resolved/archived markets. **Correct pattern:** query `GET /events?slug=<event-slug>` (event slug = market slug minus the last `-<outcome>` segment, e.g. `ucl-rma1-bay1-2026-04-07-bay1` → `ucl-rma1-bay1-2026-04-07`). Resolution signal: `event.closed=true` + `outcomePrices: ["1","0"]` (YES won) or `["0","1"]` (YES lost). Polymarket price-settles on-chain before officially setting `resolved=true` + `winnerOutcome` (can lag days). Script uses price-settlement as primary signal with official resolution as fallback.
- **PolyHive exposure cap (2026-04-11):** `paper-trade.py` has `PAPER_EXPOSURE_CAP = 500.0`. If total open exposure ≥ $500 at execution time, the script halts and enters no new trades. `size-and-check.py` still has `PAPER_MAX_TOTAL = 2000.0` for Kelly sizing — do not change without MM approval. The two constants serve different purposes: sizing (how big a position) vs. cap (hard stop on total portfolio).
- **Telegram trusted users (2026-04-11):** Thaleia (Telegram ID `6935745875`) is a MM-approved contact with mhivebook group access in group `-5129749008` only. Documented in `workspace/USER.md` on VPS so all agents know who she is. Security policy unchanged: no other external users on any bot allowlist.
